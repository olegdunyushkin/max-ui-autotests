...
В отчёте отображаются 17 успешных сценариев, группировка по функциональности, шаги-действия,
теги, приоритет и автор тестов.

![Обзор Allure Report](docs/images/allure-overview.png)

Для каждого теста прикладываются скриншот, Page Source, логи браузера и видео исполнения.
...
После сборки отправляется отчет в ТГ бота, который содержит результат запуска и ссылку на Allure Report.
